...
def run(config: Config) -> int:
    ...
    client = GitLabClient(config.api_url, token, config.timeout_seconds)
    today = date.today()

    details, details_source = get_token_details(client, config)
    ...
    print(f"Rotating token {token_id_report}; replacement expires_at={new_expires_at}.")
